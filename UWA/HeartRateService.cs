    ...
    public class HeartRateService
    {
        ...
        public async Task InitializeServiceAsync(DeviceInformation device)
        {
            try
            {
                ...
                {
                    var dialog = new MessageDialog("Access to the device is denied, because the application was not granted access, " +
            ...
            {
                var dialog = new MessageDialog("ERROR: Accessing your device failed." + Environment.NewLine + e.Message);
                await dialog.ShowAsync();
        ...
        private async Task ConfigureServiceForNotificationsAsync()
        {
            try
            {
                ...
                //characteristic = service.GetCharacteristics(GattCharacteristicUuids.HeartRateMeasurement)[0];
